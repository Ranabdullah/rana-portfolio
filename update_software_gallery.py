  ...
  <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 16px; padding: 0 22px 32px;">
    ...
    <div class="gc sw-card" onclick="window.open('#', '_blank')" style="padding: 28px; display: flex; flex-direction: column; justify-content: space-between; min-height: 220px">
      ...
      <div style="display: flex; align-items: center; justify-content: space-between; margin-top: 20px;">
        ...
        <button class="hcta" style="padding:8px 20px; font-size:11px; border-radius:10px; transform:none">Get on Market →</button>
      </div>
    </div>
...
# 2. Update CATS array (exclude Canva 2023)
root = r'f:\AntiGravity\Apps Data\Website Portfolio Work\Photos & to upload'
cats_dict = {}
pillar_map = {
    'Client': 'client', 'Exterior': 'client', 'Interior': 'client',
    'Get Connected Interior': 'client', 'Product Render': 'client',
    'Logo': 'client', 'Rizpros': 'client', 'Eskimo': 'client', 'IDFL': 'client'
}
...
for item in os.listdir(root):
    ...
    if os.path.isdir(item_path):
        ...
        if files:
            cats_dict[cat_name] = {"name": cat_name, "pillar": pillar, "fe": cat_name, "files": files}
...
# Replace the CATS array in the HTML
html = re.sub(r'const CATS = \[.*?\];', f'const CATS = {cats_json};', html, flags=re.DOTALL)

with open(html_path, 'w', encoding='utf-8') as f:
    f.write(html)

print("HTML file updated with new software and cleaned gallery.")
